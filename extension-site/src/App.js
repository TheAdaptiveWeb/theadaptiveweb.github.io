...
class App extends Component {
		...
		this.conn = new PluginCommunicator(
			installedAdapters => {
				...
				installedAdapters.forEach(adapter => {
					if (
						this.state.adapters[adapter.id] !== undefined &&
						semver.gt(this.state.adapters[adapter.id].version, adapter.version)
					) {
	...
	removeAdapter(adapterId, developer = false) {
		...
		if (developer) {
			this.setState({
				developerAdapters: this.state.developerAdapters.filter(
					x => x.id !== adapterId
				)
			});
		}
		...
		return (
			<AppContext.Provider
				value={{
					...
					getAdapterPreferences: this.conn.getAdapterPreferences.bind(
						this.conn
					),
					updateAdapterPreferences: this.conn.updateAdapterPreferences.bind(
						this.conn
					)
				}}
			>
...
const PageContainer = styled.div`
	...
	bottom: 0;
	${props =>
		props.globalOptions.animations &&
		css`
