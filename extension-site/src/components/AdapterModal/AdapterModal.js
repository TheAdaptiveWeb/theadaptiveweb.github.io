...
class AdapterModal extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			displaySettings:
				...
				Object.keys(this.props.adapter.preferenceSchema).length !== 0
		};
	}
	...
	render() {
		return (
			...
			>
				<div>
					<UnstyledLink to="/adapters">
						<BackLink>Back to Adapters</BackLink>
					</UnstyledLink>
					<Title id="dialog-title">{this.props.adapter.name}</Title>
					<Subtitle id="dialog-description">
						{this.props.adapter.description}
					</Subtitle>
					<AdapterTags>
						{this.props.adapter.tags.map((tag, index) => (
							<AdapterTag key={`label-${index}`} label={tag} />
						))}
					</AdapterTags>
				</div>
				...
				{this.props.adapter.developer && (
					<DevContainer>
						Developer mode: Run `awcli publish` to publish
					</DevContainer>
				)}

				{(this.state.displayDescription ||
					(this.state.displaySettings && this.props.installed)) && (
					<PageContainer>
						{this.state.page === 'description' &&
						this.state.displayDescription ? (
								<ReactMarkdown
									source={this.props.adapter.about}
									escapeHtml={false}
								/>
							) : (
								<AdapterSettings
									schema={this.props.adapter.preferenceSchema}
				...
				<ActionsContainer>
					<Actions>
						...
						{this.props.adapter.developer || this.props.installed ? (
							<Button
								onClick={() =>
									this.props.removeAdapter(
										this.props.adapter.id,
										this.props.adapter.developer
									)
								}
								type="danger"
							...
							<Button
								onClick={() => this.props.installAdapter(this.props.adapter)}
							>
								Enable
							</Button>
						)}
